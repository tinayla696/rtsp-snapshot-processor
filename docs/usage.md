...
ただし、現在の `FrameReceiver` は `cv2.VideoCapture()` によるRTSP入力を実装しており、生RTP/UDPのデペイロード処理は未実装です。実機配信を使うエンドツーエンドテストには、H.264 RTPを受信・デコードするGStreamerまたはFFmpegベースの受信処理が別途必要です。
